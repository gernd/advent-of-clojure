...
do (println "steps-total " steps-total " squares " squares " current steps " current-steps " current-sprint " current-sprint " direction " direction " take " take "threshold" threshold)
  (let [position (last squares)
      ...
      (if (< current-steps current-sprint)
          (recur threshold (inc steps-total) (conj squares next-pos) direction take current-sprint (inc current-steps))
          (let [next-direction
              ...
              (if (= take 1)
                  (recur threshold (inc steps-total) (conj squares next-pos) next-direction (inc take) current-sprint 1)
              (if (= take 2)
                  (recur threshold (inc steps-total) (conj squares next-pos) next-direction 1 (inc current-sprint) 1))))))))
